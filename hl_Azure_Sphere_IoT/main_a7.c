...
static const int buffArrSize =60;
static int buffArray[60];
...
static bool SetUpAzureIoTHubClientWithDps(void)
{
    AZURE_SPHERE_PROV_RETURN_VALUE provResult =
        IoTHubDeviceClient_LL_CreateWithAzureSphereDeviceAuthProvisioning(scopeId, 70000,
                                                                          &iothubClientHandle);
...
static void SocketEventHandler(EventLoop *el, int fd, EventLoop_IoEvents events, void *context)
{
   ...
   // Log_Debug("Received %d bytes: ", bytesReceived);
    ...
    buffArray[buffCount] = m4bufferOutput;
    Log_Debug(" Buffer: %d \n", m4bufferOutput);
...
int outputValue =0;
for(int i=0; i<(buffArrSize-1); i++){
  if(buffArray[i] >80 && buffArray[i] < 500){
      Log_Debug("Array list %d \r\n\n", buffArray[i]);
      outputValue = buffArray[i] ;
      break;
  }
...
buffArray[60] = NULL;
          ...
          char tempAsString[1024];
		   sprintf(tempAsString, "{\"distance\": \"%d\", \"timestamp\": \"%s\"}", outputValue, time_stamp());
